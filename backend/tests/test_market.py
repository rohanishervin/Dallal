...
class TestMarketEndpoints:
    ...
    async def test_instruments_data_structure(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            trading_fields = [
                "contract_multiplier", "round_lot", "min_trade_vol", 
                "max_trade_volume", "trade_vol_step", "px_precision"
            ]
            ...
            currency_fields = [
                "currency_precision", "currency_sort_order",
                "settl_currency_precision", "settl_currency_sort_order"
            ]
    ...
    async def test_eurusd_symbol_availability(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            expected_non_null_fields = [
                "contract_multiplier", "round_lot", "min_trade_vol",
                "max_trade_volume", "trade_vol_step", "px_precision",
                "currency_precision", "settl_currency_precision"
            ]
    ...
    async def test_instruments_field_completeness(self):
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            all_fields = [
                "symbol", "security_id", "currency", "settle_currency", "trade_enabled",
                "contract_multiplier", "round_lot", "min_trade_vol", "max_trade_volume",
                "trade_vol_step", "px_precision", "currency_precision", "currency_sort_order",
                "settl_currency_precision", "settl_currency_sort_order", "commission",
                "comm_type", "swap_type", "default_slippage", "sort_order", "group_sort_order",
                "status_group_id", "margin_factor_fractional"
            ]
            ...
            for field in core_fields:
                ...
                assert population_rate > 0.9, f"Core field {field} only populated in {population_rate:.1%} of instruments"
            
            # Trading fields should be populated for majority of instruments
            trading_fields = ["contract_multiplier", "round_lot", "px_precision"]
            for field in trading_fields:
                population_rate = field_population[field] / total_instruments
                assert population_rate > 0.8, f"Trading field {field} only populated in {population_rate:.1%} of instruments"
