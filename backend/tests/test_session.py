    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        if session["overall_active"]:
            ...
            if session["trade_session"]:
                ...
                print(f"   Trade: {trade['connection_type']}, Active: {trade['is_active']}, Heartbeat: {trade['heartbeat_status']}")
            if session["feed_session"]:
                feed = session["feed_session"]
                print(f"   Feed: {feed['connection_type']}, Active: {feed['is_active']}, Heartbeat: {feed['heartbeat_status']}")
        else:
