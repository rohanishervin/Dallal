...
class QuickFIXFeedAdapter(fix.Application):
    def __init__(self):
        super().__init__()
    ...
    def connect(
        ...
        try:
            ...
            self.current_config_file = QuickFIXConfigManager.update_config_file("feed_session.cfg", "feed")
            ...
            logger.info(f"Connecting as {username} to feed session...")
            self.initiator.start()

            if self.logon_event.wait(timeout):
                logger.info("✓ Feed session connected successfully!")
                return True, None
            else:
                logger.error("✗ Feed session connection timeout")
                return False, "Connection timeout"
        except Exception as e:
            logger.error(f"✗ Feed session connection failed: {e}")
            return False, f"Connection failed: {e}"

    def disconnect(self) -> bool:
        try:
            if self.initiator:
                logger.info("Disconnecting feed session...")
                self.logout_event.clear()
                self.initiator.stop()
                self.logout_event.wait(10)
                logger.info("✓ Feed session disconnected")
            ...
            logger.error(f"Error disconnecting feed session: {e}")
            return False
    ...
    def onCreate(self, sessionID):
        logger.info(f"Feed session created: {sessionID}")
        self.session_id = sessionID

    def onLogon(self, sessionID):
        logger.info(f"✓ Feed session logged on: {sessionID}")
        self.logged_on = True
        self.logon_event.set()

    def onLogout(self, sessionID):
        logger.info(f"✗ Feed session logged out: {sessionID}")
        self.logged_on = False
