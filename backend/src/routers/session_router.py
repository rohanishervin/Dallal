...
import time
from src.schemas.session_schemas import SessionStatusResponse, SessionStatus, IndividualSessionStatus
...
async def get_session_status(
    ...
    if trade_details:
        trade_session_status = IndividualSessionStatus(
            ...
            last_heartbeat=datetime.fromtimestamp(trade_details["last_heartbeat"]) if trade_details["last_heartbeat"] else None,
    ...
    if feed_details:
        feed_session_status = IndividualSessionStatus(
            ...
            last_heartbeat=datetime.fromtimestamp(feed_details["last_heartbeat"]) if feed_details["last_heartbeat"] else None,
            session_age_seconds=feed_details["session_age_seconds"],
            heartbeat_status=feed_details["heartbeat_status"]
        )
    
    # Determine overall status
    overall_active = bool((trade_details and trade_details["is_active"]) or (feed_details and feed_details["is_active"]))
